...
use crate::{
    render::sprite_batch::{prepare_sprite_data, SpriteBatchRenderPass, SpriteData, Vertex},
...
impl Graphics {
    pub async fn new(window: Window, base_path: String) -> Self {
        ...
        let depth_texture =
            resources::texture::Texture::create_depth_texture(&device, &config, "depth_texture");
        Self {
            window,
            surface,
            device,
    ...
    pub(crate) fn resize(&mut self, new_size: &winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.depth_texture = resources::texture::Texture::create_depth_texture(
                &self.device,
                &self.config,
                "depth_texture",
            );
